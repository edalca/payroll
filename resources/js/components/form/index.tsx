...
const FormBuilder = ({ user, form: formBuilder, action }: FormBuilderProps) => {
    const { useFormBuild, useFormSchema } = useFormBuilder(formBuilder.fields);
    const build = useFormBuild();
    const schema = useFormSchema(formBuilder.fields); // Quité useMemo porque useInitDataForm debería memoizarse internamente si es necesario
    ...
    const onSubmit = async () => {
        ...
        if (action === "New") {
            ...
            router.put(`/${formBuilder.name}/${model.id}`, model, {
                onSuccess() {
        ...
        >
            <form >
                        ...
                        <TabsList>
                            {
                                build.children.map((tab, idx) => (
                                    <TabsTrigger selected={idx == 0 ? true : false} key={"tabsList-" + tab.fieldname} value={tab.fieldname}>{tab.label}</TabsTrigger>
                                ))
                            }
                        </TabsList>

                        {build.children.map((tab, tidx) => (
                            <TabsContent key={"tabsContent-" + tab.fieldname} value={tab.fieldname}>{
                                tab.children.map((section, sidx) => {
                                    const count = section.children.length;
                                    const grid = gridOptions[count] || "grid-cols-2"; // Default a 2 si no está definido
                                    return (
                                        <div
                                            key={section.fieldname}
                                            className={cn("grid gap-4", grid)} // Agregué gap-4 para espaciado
                                        >
                ...
                {build.type === "Section Break" && (
                    <div className={
                        cn("w-full flex flex-col bg-white border border-t-4 border-t-blue-600 shadow-sm rounded-xl dark:bg-neutral-900 dark:border-neutral-700 dark:border-t-blue-500 dark:shadow-neutral-700/70",
                            "p-4 md:p-5")}>
                        {
                            build.children.map((section, idx) => {
                                const count = section.children.length;
                                const grid = gridOptions[count] || "grid-cols-2"; // Default a 2 si no está definido
                                return (
                                    <div
